...
<!--		<h2 class="woocommerce-Reviews-title">-->
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		<div id="review_form_wrapper">
			<div id="review_form" class="review_grids">
				<?php
...
//				    var_dump($key , '-----------------------');
				    ...
				    if($key === "author"){
					    $field_html  = '<div class="comment-form-author-email-wrap" ><p class="comment-form-' . esc_attr( $key ) . '">';
...
//					$field_html .= '</label><input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" type="' . esc_attr( $field['type'] ) . '" value="' . esc_attr( $field['value'] ) . '" size="30" ' . ( $field['required'] ? 'required' : '' ) . ' /></p>';
				...
				if($key === "email"){
					$field_html .= '<input placeholder="'.esc_html( $field['label'] ).'" id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" type="' . esc_attr( $field['type'] ) . '" value="' . esc_attr( $field['value'] ) . '" size="30" ' . ( $field['required'] ? 'required' : '' ) . ' /></p> </div>';
				}
				elseif ($key === 'author'){
